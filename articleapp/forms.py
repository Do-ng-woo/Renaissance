...
class ArticleCreationForm(ModelForm):
    ...
    class Meta:
        model = Article
        fields = ['title', 'image', 'project', 'artist','content', 'datetime','hide']
...
class ArticleSearchForm(forms.Form):
    ...
    search_field = forms.ChoiceField(choices=SEARCH_FIELDS, required=False, label='검색할 필드')
    search_keyword = forms.CharField(max_length=255, required=False, label='검색어')
    date_range = forms.DateField(
        widget=DateRangePickerInput(),
        required=False, label='날짜 범위'
    )
